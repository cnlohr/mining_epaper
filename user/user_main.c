 ...
 * @param arg unused
 */
static void ICACHE_FLASH_ATTR timer10ms(void *arg)
{
	...
	TickServer();
	{
		...
		do
		{
			r = SQUEEPAPER_Poll(10);
			opsct++;
			if( r == 1 ) return;
			if( opsct == 10000 ) break;
		} while( r > 1 );
	}
...
void ICACHE_FLASH_ATTR user_init(void)
{
	...
	// Set timer10ms to be called every 100ms
	os_timer_disarm(&some_timer);
	os_timer_setfn(&some_timer, (os_timer_func_t *)timer10ms, NULL);
